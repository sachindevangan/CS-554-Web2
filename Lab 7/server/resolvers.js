...
getBookById: async (_, args) => {
  ...
  return book;
    ...
    booksByGenre: async (_, { genre }) => {
      // trim the grenre
        ...
        // Check if the genre contains any digits
        if (/\d/.test(lowerGenre)) {
          throw new GraphQLError('Genre cannot contain numbers', {
            extensions: { code: 'INVALID', statusCode: 400 }
          });
        }
      ...
      booksByPriceRange: async (_, { min, max }) => {
        ...
        searchAuthorsByName: async (_, { searchTerm }) => {
          ...
          if (!isNaN(parseFloat(lowercaseSearchTerm)) && isFinite(lowercaseSearchTerm)) {
            throw new GraphQLError('SearchTerm cannot be a number', {
              extensions: { code: 'INVALID', statusCode: 400 }
            });
          }
  ...
  Mutation: {
    addAuthor: async (_, args) => {
      ...
      if (!first_name.trim() || !last_name.trim() || !date_of_birth.trim() || !hometownCity.trim() || !hometownState.trim() || !regForName.test(first_name) || !regForName.test(last_name) ||  first_name.trim().length < 2 || last_name.trim().length < 2 || hometownCity.trim().length < 2) {
      
      let errorMessage = 'Invalid input values';

      if (hometownCity.trim().length < 2) {
        errorMessage = 'Hometown city cannot be less than 2 characters';
      }
      ...
      if (/^\d+$/.test(trimHometownCity)) {
        throw new GraphQLError('HometownCity cannot be an integer', {
          extensions: { code: 'BAD_REQUEST', statusCode: 400 }
        });
      }
...
const updatedCache = await client.set('authors', JSON.stringify(updatedAuthorsCache));
 ...
 return updatedAuthor;
  ...
  removeAuthor: async(_, args) => {
    ...
    const deleteBooksOfAuthor = await books.deleteMany({ authorId: trimmedId });
    ...
    // Remove author from the cache if present
    const existCachedAuthor = await client.exists(`author:${trimmedId}`);
    if (existCachedAuthor) {
      const removeCachedAuthor = await client.del(`author:${trimmedId}`);
    }
    ...
    if (existCachedBooks) {
      ...
      const updatedBooks = booksData.filter(book => book.authorId !== trimmedId);
  ...
  const invalidFields = ['genres', 'publisher', 'summary', 'language', 'format'];
     for (const field of invalidFields) {
       if (typeof args[field] === 'string' && /\d/.test(args[field])) {
         throw new GraphQLError(`${field.charAt(0).toUpperCase() + field.slice(1)} cannot contain numbers`, {
        extensions: { code: 'BAD_REQUEST', statusCode: 400 }
      });
    }
  }
...
if (!validateISBN13(trimmedISBN)) {
  ...
  editBook: async (_, args) => {
    ...
    const { publicationDate, isbn, pageCount } = updatedFields;

  const containsOnlySpaces = (str) => str.trim().length === 0;

  const invalidFields = ['title', 'publisher', 'summary', 'language'];
  for (const field of invalidFields) {
    ...
    if (typeof fieldValue === 'string') {
      ...
      if (containsOnlySpaces(fieldValue)) {
        throw new GraphQLError(`${field.charAt(0).toUpperCase() + field.slice(1)} cannot contain only spaces`, {
          extensions: { code: 'BAD_REQUEST', statusCode: 400 }
        });
      }
    }
  ...
  const arrayFields = ['genres', 'format'];
  for (const field of arrayFields) {
    const arrayValue = updatedFields[field];
    if (Array.isArray(arrayValue)) {
      for (const element of arrayValue) {
        if (typeof element === 'string' && containsOnlySpaces(element)) {
          throw new GraphQLError(`${field.charAt(0).toUpperCase() + field.slice(1)} cannot contain elements with only spaces`, {
            extensions: { code: 'BAD_REQUEST', statusCode: 400 }
          });
        }
      }
  ...
  if (!Number.isSafeInteger(pageCount) || pageCount <= 0 || pageCount < 10) {
    throw new GraphQLError('Invalid pageCount. Page count should be a positive number and preferably above 10 pages.', {
      extensions: { code: 'BAD_REQUEST', statusCode: 400 }
    });
  }
...
removeBook: async (_, args) => {
  ...
  // If the author is found, remove the book's _id from the author's books array and decrement the numOfBooks
  if (author) {
    await authors.updateOne({ _id: book.authorId }, { $pull: { books: trimmedId }, $inc: { numOfBooks: -1 } });
  }
  ...
  // Remove book from the cache if present
  const existCachedBook = await client.exists(`book:${trimmedId}`);
  if (existCachedBook) {
    const removeCachedBook = await client.del(`book:${trimmedId}`);
  }

  // Update books cache by removing the current book
  const existCachedBooks = await client.exists('books');
  if (existCachedBooks) {
    const cachedBooks = await client.get('books');
    const booksData = JSON.parse(cachedBooks);
    ...
    // Update authors cache and remove the bookId and decrement the numOfBooks
    const existCachedAuthors = await client.exists('authors');
    if (existCachedAuthors) {
      const cachedAuthors = await client.get('authors');
      const authorsData = JSON.parse(cachedAuthors);

      // Find the author in the array and update the cache
      const updatedAuthors = authorsData.map(author => {
        if (author._id === book.authorId) {
          // Filter out the specific book ID from the author's books array
          author.books = author.books.filter(bookId => bookId !== trimmedId);
